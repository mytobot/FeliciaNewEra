...
`Health Poinmu Kurang Dari 80
Silahkan Heal Untuk Berpetualang Lagi.`.trim(), imgr + 'lowhealth', [
[`𝗛𝗘𝗔𝗟`, `${usedPrefix}heal`]
], m)
    if (new Date - user.lastadventure <= cooldown) return conn.sendButton(m.chat, 
`${htki} COOLDOWN ${htka}`,
`*Kmu Sudah Berpetualangan, Harap Tunggu Sebentar Lagi*

⏱️ ${timers.toTimeString()}`.trim(), imgr + 'cooldown', [
[`𝗜𝗡𝗩𝗘𝗡𝗧𝗢𝗥𝗬`, `${usedPrefix}inventory`],
[`𝗗𝗔𝗜𝗟𝗬`, `${usedPrefix}daily`]
], m)
